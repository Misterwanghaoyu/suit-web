...
import { addShirtAction, updateShirtsAction, deleteShirtAction } from '@/actions/admin/shirts';
...
export default function ShirtsClient({ initialData }: ShirtsClientProps) {
  ...
  const handleDelete = async (item: Product) => {
    if (confirm(`确定要删除衬衫 ${item.name} 吗？`)) {
      const result = await deleteShirtAction(item.id);
      if (result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      const result = await updateShirtsAction(updatedData);
      if (result.success) {
        setData(updatedData);
      }
    } else {
      const result = await addShirtAction(newItem);
  ...
  return (
    <div>
      <div className="mb-4">
        <Button onClick={handleCreate}>添加衬衫</Button>
      </div>
      <DataTable data={data} columns={columns} onEdit={handleEdit} onDelete={handleDelete} />
      
      <Dialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} title={editingItem ? '编辑衬衫' : '添加衬衫'}>
