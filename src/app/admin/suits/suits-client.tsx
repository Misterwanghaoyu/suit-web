...
import { addSuitAction, updateSuitsAction, deleteSuitAction } from '@/actions/admin/suits';
import { Product } from '@/lib/data';

interface SuitsClientProps {
  initialData: Product[];
}

export default function SuitsClient({ initialData }: SuitsClientProps) {
  ...
  const handleDelete = async (item: Product) => {
    if (confirm(`确定要删除西装 ${item.name} 吗？`)) {
      const result = await deleteSuitAction(item.id);
      if (result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      const result = await updateSuitsAction(updatedData);
      if (result.success) {
        setData(updatedData);
      }
    } else {
      const result = await addSuitAction(newItem);
  ...
  return (
    <div>
      <div className="mb-4">
        <Button onClick={handleCreate}>添加西装</Button>
      </div>
      <DataTable data={data} columns={columns} onEdit={handleEdit} onDelete={handleDelete} />
      
      <Dialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} title={editingItem ? '编辑西装' : '添加西装'}>
