...
import { addAccessoryAction, updateAccessoriesAction, deleteAccessoryAction } from '@/actions/admin/accessories';
...
export default function AccessoriesClient({ initialData }: AccessoriesClientProps) {
  ...
  const handleDelete = async (item: Product) => {
    if (confirm(`确定要删除配饰 ${item.name} 吗？`)) {
      const result = await deleteAccessoryAction(item.id);
      if (result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      const result = await updateAccessoriesAction(updatedData);
      if (result.success) {
        setData(updatedData);
      }
    } else {
      const result = await addAccessoryAction(newItem);
  ...
  return (
    <div>
      <div className="mb-4">
        <Button onClick={handleCreate}>添加配饰</Button>
      </div>
      <DataTable data={data} columns={columns} onEdit={handleEdit} onDelete={handleDelete} />
      
      <Dialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} title={editingItem ? '编辑配饰' : '添加配饰'}>
