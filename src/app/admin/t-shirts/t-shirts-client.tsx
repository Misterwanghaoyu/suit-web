...
import { addTShirtAction, updateTShirtsAction, deleteTShirtAction } from '@/actions/admin/t-shirts';
import { Product } from '@/lib/data';

interface TShirtsClientProps {
  initialData: Product[];
}

export default function TShirtsClient({ initialData }: TShirtsClientProps) {
  ...
  const handleDelete = async (item: Product) => {
    if (confirm(`确定要删除T恤 ${item.name} 吗？`)) {
      const result = await deleteTShirtAction(item.id);
      if (result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      const result = await updateTShirtsAction(updatedData);
      if (result.success) {
        setData(updatedData);
      }
    } else {
      const result = await addTShirtAction(newItem);
  ...
  return (
    <div>
      <div className="mb-4">
        <Button onClick={handleCreate}>添加T恤</Button>
      </div>
      <DataTable data={data} columns={columns} onEdit={handleEdit} onDelete={handleDelete} />
      
      <Dialog isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} title={editingItem ? '编辑T恤' : '添加T恤'}>
